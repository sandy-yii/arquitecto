// Filas extraídas del Reglamento de Construcciones y Normas Técnicas
// para el Municipio de Chihuahua. Cada fila: [espacio/elemento, artículo, descripción]
module.exports = [
  // --- Criterios generales del proyecto ---
  ["Altura de la construcción", "Artículo 2", "Se define como la dimensión vertical de la construcción, tomada a partir del nivel medio longitudinal de la guarnición hacia arriba."],
  ["Altura máxima permitida", "Artículo 2", "Es la medida en metros que fijan los planes o programas, contada desde el nivel medio longitudinal de la guarnición hasta la parte más alta de la construcción."],
  ["Condiciones del proyecto", "Artículo 52", "El proyecto arquitectónico debe garantizar habitabilidad, funcionamiento, higiene, imagen urbana, comunicación, seguridad estructural, integración al contexto y acondicionamiento ambiental."],

  // --- Programa de la vivienda ---
  ["Vivienda mínima", "Artículo 53", "El diseño arquitectónico y estructural debe permitir que la casa crezca hasta integrar tres recámaras, dos baños, sala, comedor y cocina."],

  // --- Dimensiones mínimas, artículo 54 ---
  ["Recámara (área)", "Artículo 54", "Mínimo 7.00 m²."],
  ["Recámara (lado)", "Artículo 54", "El lado libre no baja de 2.70 m."],
  ["Recámara (altura)", "Artículo 54", "Altura libre mínima de 2.50 m."],
  ["Alcoba (área)", "Artículo 54", "Mínimo 6.00 m²."],
  ["Alcoba (lado)", "Artículo 54", "Lado mínimo de 2.00 m."],
  ["Alcoba (altura)", "Artículo 54", "Altura mínima de 2.50 m."],
  ["Estancia o sala (área)", "Artículo 54", "Mínimo 7.30 m²."],
  ["Estancia (lado)", "Artículo 54", "Lado mínimo de 2.70 m."],
  ["Estancia (altura)", "Artículo 54", "Altura mínima de 2.50 m."],
  ["Comedor (área)", "Artículo 54", "Mínimo 6.30 m²."],
  ["Comedor (lado)", "Artículo 54", "Lado mínimo de 2.70 m."],
  ["Comedor (altura)", "Artículo 54", "Altura mínima de 2.50 m."],
  ["Estancia-comedor integral (área)", "Artículo 54", "Cuando los dos espacios van integrados, el área mínima es de 13.60 m²."],
  ["Estancia-comedor integral (lado)", "Artículo 54", "Lado mínimo de 2.70 m."],
  ["Estancia-comedor integral (altura)", "Artículo 54", "Altura mínima de 2.50 m."],
  ["Cocina (área)", "Artículo 54", "Mínimo 3.00 m²."],
  ["Cocina (lado)", "Artículo 54", "Lado mínimo de 1.50 m. La observación “a” de la tabla aclara que esa dimensión de lado se refiere a la longitud de la cocina."],
  ["Cocina (altura)", "Artículo 54", "Altura mínima de 2.50 m."],
  ["Cocineta integrada (lado)", "Artículo 54", "Si la cocineta va integrada a la estancia-comedor, el lado mínimo es de 2.00 m."],
  ["Cocineta integrada (altura)", "Artículo 54", "Altura mínima de 2.50 m."],
  ["Cuarto de lavado (área)", "Artículo 54", "Mínimo 1.68 m²."],
  ["Cuarto de lavado (lado)", "Artículo 54", "Lado mínimo de 1.40 m."],
  ["Cuarto de lavado (altura)", "Artículo 54", "Altura mínima de 2.30 m."],
  ["Cuarto de aseo y despensa", "Artículo 54", "La tabla sólo les fija altura mínima de 2.30 m; no marca área ni lado."],
  ["Baños y sanitarios (altura)", "Artículo 54", "La tabla sólo les fija altura mínima de 2.30 m; tampoco marca área ni lado."],
  ["Muebles del baño", "Artículo 54", "La observación “b” remite las dimensiones libres mínimas de los muebles sanitarios al Título Cuarto del Reglamento."],

  // --- Predio y área libre ---
  ["Área libre, predio menor de 500 m²", "Artículo 55", "Se deja sin construir el 20 por ciento."],
  ["Área libre, de 500 a 2,000 m²", "Artículo 55", "Se deja sin construir el 22.5 por ciento."],
  ["Área libre, de 2,000 a 3,500 m²", "Artículo 55", "Se deja sin construir el 25 por ciento."],
  ["Área libre, de 3,500 a 5,500 m²", "Artículo 55", "Se deja sin construir el 27.5 por ciento."],
  ["Área libre, más de 5,500 m²", "Artículo 55", "Se deja sin construir el 30 por ciento."],
  ["Pavimento del área libre", "Artículo 55", "Sólo se puede pavimentar con materiales que permitan la filtración del agua."],
  ["Arborización", "Artículo 55", "El área libre se arboriza conforme a la tabla de vegetación, con plantas de 1.5 m de altura mínima. En zonificación H4 corresponden 2 árboles por lote y hasta 30 por ciento del área libre pavimentada."],
  ["Ubicación de los árboles", "Artículo 55", "Van al frente del lote sobre la vía pública, sin obstruir el espacio peatonal."],

  // --- Alturas y colindancias ---
  ["Altura máxima del edificio", "Artículo 56", "Ningún punto puede estar más alto que dos veces su distancia mínima al plano vertical sobre el alineamiento de la acera opuesta."],
  ["Medición de la altura", "Artículo 57", "Se mide a partir de la cota media de la guarnición de la acera en el frente del predio."],
  ["Predio en esquina", "Artículo 58", "Sobre la calle angosta se puede usar la altura que corresponde a la calle ancha, hasta una distancia de dos veces el ancho de la calle angosta medida desde la esquina."],
  ["Restricción hacia el norte", "Artículo 59", "Si el límite es orientación norte y colinda con predios habitacionales, se deja una restricción del 15 por ciento de la altura máxima."],
  ["Separación por sismo", "Artículo 60", "Hay que verificar la separación con predios y edificaciones colindantes según el artículo de separaciones mínimas de diseño por sismo."],
  ["Separación entre edificios plurifamiliares", "Artículo 61", "Hasta 50 viviendas, la separación es cuando menos la dimensión mínima que se pide para patios de iluminación y ventilación, según el tipo de local y la altura promedio de los paramentos."],
  ["Separación en conjuntos habitacionales", "Artículo 62", "En más de 50 viviendas, la separación en dirección norte-sur es por lo menos del 60 por ciento de la altura promedio de los edificios, y en dirección este-oeste por lo menos del 100 por ciento."],
  ["Barda del predio", "Artículo 63", "En cualquier edificación se construye una barda que delimite el predio, con altura mínima de un metro."],
  ["Tapajuntas", "Artículo 63", "Si ya existe construcción colindante se coloca tapajuntas y se resuelve la infiltración del agua."],
  ["Protección de la colindancia", "Artículo 237", "Las separaciones se protegen con tapajuntas o botaguas que impidan la entrada de agua y basura, y se garantiza el aislamiento acústico entre edificaciones."],

  // --- Cochera y estacionamiento ---
  ["Cajones, casa hasta 120 m²", "Artículo 64", "Un cajón por cada vivienda."],
  ["Cajones, casa de más de 120 hasta 250 m²", "Artículo 64", "Dos cajones por cada vivienda."],
  ["Cajones, casa de más de 300 m²", "Artículo 64", "Tres cajones por cada vivienda."],
  ["Cajones, plurifamiliar hasta 60 m²", "Artículo 64", "Un cajón por cada vivienda."],
  ["Cajones, plurifamiliar de 60 a 120 m²", "Artículo 64", "1.25 cajones por cada vivienda."],
  ["Cajones para empleados", "Artículo 64", "Al resultado de la tabla se le suma un incremento del 10 por ciento."],
  ["Medida del cajón", "Artículo 65", "5.50 por 2.70 metros."],
  ["Cajón en cordón", "Artículo 65", "7.00 por 2.40 metros, sin contar las áreas de circulación."],
  ["Cajón para personas con discapacidad", "Artículo 65", "Uno por cada 50 cajones o fracción, de 5.50 por 3.80 metros, ubicado lo más cerca posible de la entrada."],
  ["Señalización del cajón accesible", "Artículo 65", "Lleva franja de circulación señalizada, señales en poste y en piso, y tope para vehículos."],
  ["Circulación con cajones a 90 grados", "Artículo 65", "Ancho mínimo de 7.20 metros lineales."],
  ["Circulación a 30 y 45 grados", "Artículo 65", "En un sentido, mínimo 3.50 metros lineales."],
  ["Circulación a 60 grados", "Artículo 65", "Mínimo 4.50 metros lineales."],
  ["Circulación en dos sentidos", "Artículo 65", "Para acomodo a 30, 45 y 60 grados, mínimo 7.00 metros lineales."],
  ["Estacionamiento de servicio privado", "Artículo 67", "No se le exigen carriles separados, áreas de recepción y entrega de vehículos ni casetas de control."],

  // --- Basura ---
  ["Depósito de basura", "Artículo 68", "Se asigna un espacio ventilado y a prueba de roedores, colocado de forma que facilite el acceso del servicio de recolección."],
  ["Índice del depósito de basura", "Artículo 69", "En habitacional plurifamiliar se calculan 0.005 metros cúbicos por cada metro cuadrado construido."],

  // --- Circulaciones ---
  ["Pasillo interior de la vivienda (ancho)", "Artículo 70", "Ancho mínimo de 0.90 m."],
  ["Pasillo interior de la vivienda (altura)", "Artículo 70", "Altura mínima de 2.30 m."],
  ["Pasillo común a dos o más viviendas (ancho)", "Artículo 70", "Ancho mínimo de 1.20 m."],
  ["Pasillo común a dos o más viviendas (altura)", "Artículo 70", "Altura mínima de 2.30 m."],
  ["Longitud del pasillo", "Artículo 70", "Estas medidas aplican cuando la circulación no excede de 6.00 m de longitud."],
  ["Distancia a una salida", "Artículo 71", "Desde cualquier punto interior hasta una puerta, circulación horizontal, escalera o rampa, máximo 30 metros medidos sobre la línea de recorrido."],
  ["Aumento de la distancia a la salida", "Artículo 72", "Esa distancia se puede incrementar hasta un 50 por ciento si la edificación cuenta con sistema de extinción de fuego."],

  // --- Escaleras ---
  ["Escalera o rampa", "Artículo 74", "Toda edificación comunica sus niveles con escaleras o rampas peatonales, aunque tenga elevador o montacargas."],
  ["Escalera privada con muro de un solo lado", "Artículo 74", "Ancho mínimo de 1.00 metro."],
  ["Escalera privada entre dos muros", "Artículo 74", "Ancho mínimo de 1.20 metros."],
  ["Escalera común a dos o más viviendas", "Artículo 74", "Ancho mínimo de 1.20 metros."],
  ["Ancho por número de usuarios", "Artículo 74", "El ancho se incrementa 0.60 m por cada 75 usuarios o fracción."],
  ["Peraltes entre descansos", "Artículo 74", "Máximo 16 peraltes en vivienda y 13 en edificios públicos."],
  ["Descanso", "Artículo 74", "Su ancho es cuando menos igual al ancho reglamentario de la escalera, y su longitud de 90 cm cuando la escalera es de una sola rampa."],
  ["Huella", "Artículo 74", "Ancho mínimo de 30 centímetros, medida entre las proyecciones verticales de los peraltes inmediatos."],
  ["Peralte", "Artículo 74", "Máximo 18 centímetros y mínimo 14 centímetros."],
  ["Peralte en escalera de servicio", "Artículo 74", "En escaleras de servicio de uso limitado el peralte puede llegar hasta 20 centímetros."],
  ["Relación huella-peralte", "Artículo 74", "Dos peraltes más una huella deben sumar cuando menos 61 centímetros y no más de 65."],
  ["Uniformidad del tramo", "Artículo 74", "Dentro de un mismo tramo la huella y el peralte conservan siempre las mismas dimensiones."],
  ["Escalera de caracol", "Artículo 74", "Diámetro mínimo de 1.20 metros."],
  ["Escalera compensada", "Artículo 74", "Huella mínima de 30 cm medida a 40 cm del barandal del lado interior, ancho máximo de 150 cm, y prohibida en edificaciones de más de cinco niveles."],
  ["Superficie del escalón", "Artículo 74", "Los escalones deben ser firmes y con superficie antiderrapante."],
  ["Nariz del escalón", "Artículo 74", "No se permiten aristas vivas ni narices sobresalientes; la arista va redondeada."],
  ["Pasamanos", "Artículo 74", "Se colocan a 75 y 90 centímetros de altura, volados 10 centímetros en los extremos."],
  ["Barandal", "Artículo 74", "En edificios públicos va cuando menos en uno de los lados, a 90 cm medidos desde la nariz del escalón y diseñado para que no pasen niños entre él."],
  ["Altura libre bajo la escalera", "Artículo 74", "En las circulaciones bajo la escalera se coloca una barrera a partir de donde la altura libre baja de 1.90 metros."],
  ["Área de aproximación", "Artículo 74", "Se marca un área de 75 centímetros como mínimo, con cambio de textura en el piso."],
  ["Puerta en cubo cerrado", "Artículo 74", "Las escaleras en cubo cerrado de cinco niveles o más llevan puertas hacia los vestíbulos en cada nivel."],

  // --- Rampas ---
  ["Rampa peatonal", "Artículo 75", "Pendiente máxima de 8 por ciento, pavimento antiderrapante y barandal cuando menos en uno de sus lados."],
  ["Ancho de la rampa", "Artículo 75", "Se aplican los mismos anchos mínimos que se establecen para las escaleras."],
  ["Descansos de rampa accesible", "Artículo 76", "La longitud máxima entre descansos es de 4.50 m, y el descanso mide cuando menos lo que la rampa, nunca menos de 1.20 m."],
  ["Bordes y pasamanos de rampa accesible", "Artículo 76", "Pendiente máxima de 8 por ciento, superficie antiderrapante, borde de protección de 5 por 5 centímetros y pasamanos a 75 y 90 cm, volados 30 cm en los extremos."],
  ["Altura libre bajo la rampa", "Artículo 76", "Se coloca una barrera a partir de donde queden menos de 1.90 metros de altura libre."],

  // --- Puertas ---
  ["Altura de las puertas", "Artículo 77", "Las puertas de acceso, intercomunicación y salida llevan 2.03 metros de altura cuando menos."],
  ["Puerta de acceso principal", "Artículo 77", "Ancho mínimo de 0.90 metros."],
  ["Puerta de locales habitables", "Artículo 77", "Ancho mínimo de 0.75 m."],
  ["Puerta de cocina", "Artículo 77", "Ancho mínimo de 0.75 m."],
  ["Puerta de locales complementarios", "Artículo 77", "Ancho mínimo de 0.60 m."],
  ["Puerta hacia la vía pública", "Artículo 77", "Su anchura total es por lo menos 1.25 veces la suma de las anchuras reglamentarias de las puertas entre vestíbulo y salas."],

  // --- Salidas ---
  ["Salida de emergencia", "Artículo 78", "Debe permitir desalojar cada nivel sin atravesar locales de servicio como cocinas y bodegas."],
  ["Puerta que da a la escalera", "Artículo 78", "Abre sobre un descanso cuya longitud de ancho no será menor que el ancho de la puerta."],
  ["Apertura de la puerta de salida", "Artículo 78", "Lleva un mecanismo que permita abrirla desde dentro con un simple empuje, y la fuerza para abrirla por completo no debe exceder de 20 kilogramos."],

  // --- Iluminación y ventanas ---
  ["Iluminación natural", "Artículo 83", "Los locales habitables y la cocina doméstica se iluminan con ventanas que den directamente a la vía pública, terrazas, azoteas, superficies descubiertas o patios."],
  ["Ventanas hacia la colindancia", "Artículo 83", "Por ningún motivo se permiten, salvo lo que autoriza el artículo 86."],
  ["Área de ventana", "Artículo 84", "No será inferior al 10 por ciento de la superficie del local."],
  ["Ventanas con distintas orientaciones", "Artículo 84", "Se divide la superficie del local entre el número de ventanas y a ese resultado se le aplica el porcentaje mínimo."],
  ["Ventana bajo marquesina o volado", "Artículo 84", "Se considera iluminada naturalmente si queda remetida como máximo el equivalente a la altura de piso a techo del local."],
  ["Domos y tragaluces", "Artículo 85", "Se permiten para iluminar baños, cocinas no domésticas, locales de trabajo, almacenamiento, circulaciones y servicios."],
  ["Medida del domo o tragaluz", "Artículo 85", "Se toma el 25 por ciento de la superficie de la ventana requerida, pero nunca menos de 45 por 45 centímetros."],
  ["Bloque de vidrio en colindancia", "Artículo 86", "Se permite iluminar con bloques de vidrio prismático traslúcido a partir de 7.20 metros sobre el nivel de banqueta, sin que sustituya las ventanas y domos mínimos."],
  ["Iluminación artificial", "Artículo 87", "Todos los locales habitables llevan iluminación nocturna artificial conforme al capítulo de instalaciones eléctricas."],
  ["Vidrios de piso a techo", "Artículo 92", "Llevan señales visuales que informen que están ahí y protección para evitar que la gente choque contra ellos."],

  // --- Patios ---
  ["Patio de iluminación y ventilación", "Artículo 87", "Su dimensión nunca será menor de 2.50 metros."],
  ["Patio para locales habitables", "Artículo 87", "La dimensión mínima es un tercio de la altura de los paramentos del patio."],
  ["Patio para locales complementarios", "Artículo 87", "La dimensión mínima es un cuarto de la altura de los paramentos."],
  ["Patio para cualquier otro local", "Artículo 87", "La dimensión mínima es un quinto de la altura de los paramentos."],
  ["Paramentos de distinta altura", "Artículo 87", "Cuando las alturas son variables se toma el promedio de las dos más altas."],
  ["Tolerancia del patio", "Artículo 87", "Se puede reducir hasta una quinta parte una de las dimensiones mínimas, siempre que la opuesta tenga una quinta parte más de lo que le corresponde."],
  ["Patio abierto a la vía pública", "Artículo 87", "Se reduce hasta la mitad la dimensión mínima en los lados perpendiculares a la vía pública."],
  ["Descuento de altura en el patio", "Artículo 87", "Del total de los paramentos se descuentan la planta baja y los niveles inmediatos superiores que sirvan de vestíbulo, estacionamiento o locales de servicio."],
  ["Forma del patio", "Artículo 87", "Lo que dice el artículo aplica a patios de base cuadrada o rectangular."],

  // --- Ventilación ---
  ["Ventilación natural", "Artículo 88", "El área de abertura de ventilación no será inferior al 5 por ciento del área del local."],
  ["Sanitario doméstico", "Artículo 89", "Si se ventila con medios artificiales, 6 cambios de aire por hora."],
  ["Cocina doméstica", "Artículo 89", "Si se ventila con medios artificiales, 10 cambios de aire por hora."],
  ["Corredores y pasillos", "Artículo 89", "Un cambio de aire por hora, y puede ser a través de otros locales."],

  // --- Alberca ---
  ["Alberca, andadores", "Artículo 94", "Llevan superficie áspera o de material antiderrapante en las orillas, construidos de manera que no se hagan encharcamientos."],
  ["Alberca, escaleras", "Artículo 94", "Donde la profundidad sea mayor de 90 centímetros se pone una escalera por cada 25 metros lineales de perímetro."],

  // --- Fachadas e imagen urbana ---
  ["Fachada en zona de patrimonio", "Artículo 96", "En zonas de patrimonio histórico, artístico o arqueológico se respetan las restricciones de altura, materiales, acabados, colores y aberturas que señalen el INAH, el INBAL y el Municipio."],
  ["Estudio de imagen urbana", "Artículo 97", "Se acompaña el levantamiento de las fachadas de la manzana y de las construcciones vecinas, el reporte fotográfico y la justificación de cómo se integra el proyecto al entorno."],
  ["Proyección de sombras", "Artículo 98", "Las edificaciones de diez metros o más sobre el nivel de banqueta presentan el estudio de las sombras que provocarían sobre los predios vecinos a lo largo del día y del año."],
  ["Vidrio reflejante en fachada", "Artículo 99", "Se permite siempre que se demuestre con estudios de asoleamiento y reflexión espectral que no provoca deslumbramientos ni aumenta la carga térmica de las edificaciones vecinas."],
  ["Fachada de colindancia", "Artículo 100", "Las de cinco niveles o más que formen parte de los paramentos de patios de iluminación y ventilación vecinos llevan acabados de colores claros."],

  // --- Cargas que se toman en cuenta ---
  ["Carga viva en casa habitación", "Artículo 132", "La Tabla 11 fija 200 kg/m² como carga uniforme mínima para casas habitación."],
  ["Carga concentrada", "Artículo 133", "Se supone repartida en un área de 0.50 m², y para ocupaciones distintas al comercio, industria, bodega y estacionamiento público es de 100 kilogramos."],
  ["Carga viva reducida", "Artículo 136", "Los elementos con área de influencia mayor a 36 m² se pueden diseñar con carga viva reducida según la ecuación del artículo."],
  ["Límite de la reducción", "Artículo 137", "La carga viva reducida no baja del 50 por ciento cuando el miembro soporta un piso, ni del 40 por ciento en los demás casos."],
  ["Carga viva en techo", "Artículo 138", "Se calcula con la fórmula Vt = 100 R1R2 y nunca resulta menor de 60 kg/m²."],
  ["Techo transitable", "Artículo 139", "Si se usa como paso peatonal se diseña para 300 kg/m², y si se usa como jardín o área de reunión para 500 kg/m²."],

  // --- Sismo ---
  ["Zona sísmica", "Artículo 169", "Chihuahua y sus zonas circunvecinas se consideran áreas con actividad sísmica posible, así que la construcción lleva un sistema capaz de resistir cargas laterales."],
  ["Coeficiente sísmico", "Artículo 171", "Es el cociente entre la fuerza cortante horizontal en la base de la estructura y el peso de la misma."],
  ["Desplazamiento lateral", "Artículo 175", "No debe exceder de 0.006 veces la diferencia de elevaciones entre entrepisos, y puede subir a 0.012 veces cuando los elementos aguanten deformaciones sin daños importantes."],
  ["Muro de sótano", "Artículo 177", "Se diseña considerando la presión lateral del suelo adyacente y el aumento de presión por sobrecargas fijas o móviles."],
  ["Losa bajo el nivel del terreno", "Artículo 178", "Se toma en cuenta la subpresión del agua, midiendo la carga hidrostática desde el desplante de la losa hasta el nivel freático."],

  // --- Techos y cimentación ---
  ["Carga de nieve", "Artículo 179", "Por el clima de Chihuahua se recomienda una carga de nieve no menor a 40 kg/m²."],
  ["Nieve acumulada", "Artículo 179", "Donde el techo o alguna obstrucción provoque acumulación, la carga mínima sube a 60 kg/m²."],
  ["Drenaje de techos", "Artículo 180", "Además del drenaje primario se instala uno secundario a mayor elevación, y el techo se diseña para soportar esa lámina de agua más una carga de 25 kg/m²."],
  ["Encharcamiento", "Artículo 181", "Los techos se diseñan para no fallar por acumulación progresiva de agua."],
  ["Cimentación", "Artículo 183", "Toda construcción se apoya en una cimentación apropiada, y nunca se desplanta sobre tierra vegetal ni sobre lechos sueltos."],
  ["Cimentar sobre relleno", "Artículo 184", "Sólo se acepta sobre rellenos artificiales cuando se demuestre que cumplen la capacidad de carga y estabilidad del diseño."],
  ["Revisión de colindancias", "Artículo 186", "Hay que investigar cimentación, estabilidad, asentamiento, agrietamiento y desplome de las construcciones vecinas."],
  ["Profundidad de desplante", "Artículo 187", "La subestructura se desplanta a una profundidad donde sea mínimo el deterioro del suelo por erosión o intemperismo."],

  // --- Instalaciones ---
  ["Instalaciones hidráulicas y sanitarias", "Artículo 189", "Se rigen por el Reglamento de la Junta Municipal de Agua y Saneamiento de Chihuahua."],
  ["Temperatura interior", "Artículo 190", "Se recomienda que la casa se diseñe o equipe para mantener entre 18 y 27 °C cuando afuera haya entre 0 y 36 °C."],
  ["Renovación de aire", "Artículo 190", "Al menos un volumen habitable de aire por hora, de forma natural o mecánica."],
  ["Pérdida de energía", "Artículo 190", "En casa habitación unifamiliar o multifamiliar se procura no pasar de 2 W/m³K."],
  ["Instalación de gas", "Artículo 190", "Las instalaciones de gas LP y gas natural cumplen con las normas oficiales mexicanas."],
  ["Registros de teléfono y datos", "Artículo 191", "Se dejan registros, ductos y preparaciones en cualquier edificación con más de 100 m² construidos."],
  ["Densidad de población", "Artículo 197", "En edificios para habitación se considera 1.85 personas por recámara."],
  ["Cajas de registro", "Artículo 203", "Se colocan a 0.60 metros del nivel del suelo y en lugares accesibles."],
  ["Registros por nivel", "Artículo 203", "En edificaciones para habitación puede haber un registro de distribución por cada dos niveles."],
  ["Líneas de distribución horizontal", "Artículo 204", "Van en tubería de fierro o plástico de 13 milímetros, con cajas de registro cada 20 metros como máximo a 0.60 m sobre el nivel del piso."],

  // --- Chimenea, muros y plafones ---
  ["Chimenea", "Artículo 224", "El tiro conduce humos y gases al exterior por la parte superior, con la salida a 1.50 metros sobre el nivel de la azotea y diseñada para poder deshollinarse."],
  ["Materiales junto a la chimenea", "Artículo 224", "Los materiales inflamables y los elementos decorativos quedan a no menos de 60 centímetros."],
  ["Campana de estufa", "Artículo 225", "Lleva filtros de grasa y sistema contra incendio, aunque las viviendas unifamiliares están exceptuadas."],
  ["Ductos de instalaciones", "Artículo 220", "Se prolongan y ventilan sobre la azotea más alta a la que tengan acceso."],
  ["Tiros o tolvas de basura", "Artículo 221", "Se prolongan por arriba de las azoteas, y sus compuertas evitan el paso de fuego o humo de un piso a otro."],
  ["Ductos de alta temperatura", "Artículo 214", "Los tiros de chimenea, campanas de extracción o ductos que conduzcan gases a más de 80 °C se separan mínimo 60 cm de los elementos estructurales de madera."],
  ["Espacio de la separación", "Artículo 215", "En el espacio que queda en esa separación debe permitirse la circulación del aire."],
  ["Muros interiores", "Artículo 229", "Los que separan departamentos o locales, o separan la zona de habitación de las circulaciones generales, se construyen a prueba de fuego y cubren todo el espacio vertical entre pisos."],
  ["Plafones", "Artículo 223", "Se construyen con materiales cuya resistencia al fuego sea de una hora por lo menos."],
];
